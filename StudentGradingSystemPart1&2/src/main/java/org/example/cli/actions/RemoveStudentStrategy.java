...
public class RemoveStudentStrategy implements ActionStrategy{
    private CourseService courseService;
    private User instructor;

    public RemoveStudentStrategy(CourseService courseService, User instructor){
    ...
    public Message execute(Message message) {
        ...
        try {
            courseService.unrollStudent(studentIdList, courseId, instructor.getId());
            responseBuilder.withData("success", new MessageData("Students unenrolled successfully", DataMode.OUTPUT, false));
        } catch (IllegalArgumentException e) {
            responseBuilder.withData("error", new MessageData(e.getMessage(), DataMode.OUTPUT, false));
        } catch (Exception e) {
            responseBuilder.withData("error", new MessageData("Failed to unenroll students", DataMode.OUTPUT, false));
        }
        return responseBuilder.build();
    }
